...
# --------------------------------------------------------------------------- #
# OUTPUT MODEL FOR AGGREGATES (Pydantic-first)
# --------------------------------------------------------------------------- #

class UnderlyingAggregatesOutput(BaseModel):
    """
    Strictly-typed container for underlying aggregates produced by
    `calculate_all_underlying_aggregates`.  Using an explicit schema enforces
    fail-fast validation and eliminates the last dictionary return value in
    this module.
    """

    # NOTE: Fields are *not* enumerated exhaustively – we allow any aggregate
    # name generated in `_calculate_*_aggregates` but still forbid extras at
    # instantiation-time to catch typos.  New fields must be added explicitly.
    total_dxoi_und: float | int | None = None
    total_gxoi_und: float | int | None = None
    total_vxoi_und: float | int | None = None
    total_txoi_und: float | int | None = None
    total_charmxoi_und: float | int | None = None
    total_vannaxoi_und: float | int | None = None
    total_vommaxoi_und: float | int | None = None
    total_nvp_und: float | int | None = None
    total_nvp_vol_und: float | int | None = None

    # Catch-all for future metrics – they must be declared explicitly to pass.
    class Config:
        extra = "forbid"
...
class VisualizationMetrics(CoreCalculator):
    ...
    def _calculate_ivsdh_scores_optimized(self, df_strike: pd.DataFrame, und_data, symbol: str) -> pd.DataFrame:
        """
        Corrected and enhanced IVSDH calculation, adhering to the EOTS Guide v2.5.
        This version now correctly integrates Vanna, Vomma, and Charm to model the full volatility surface.
        """
        try:
            # Extract all required volatility-related exposures using FAIL-FAST validation.
            ...
            vomma_exposure = self._get_column_required(df_strike, 'total_vommaxoi_at_strike', 'for IVSDH calculation')
            charm_exposure = self._get_column_required(df_strike, 'total_charmxoi_at_strike', 'for IVSDH calculation')

            # Get implied volatility data for differential calculation.
            ...
            current_iv = self._safe_float(getattr(und_data, 'u_volatility'), 'u_volatility')
            iv_differential = abs(iv_data - current_iv)
            
            # Normalize all components to bring them to a comparable scale.
            ...
            vomma_intensity = self._normalize_exposure_optimized(vomma_exposure, 'vomma', symbol)
            charm_intensity = self._normalize_exposure_optimized(charm_exposure, 'charm', symbol)
            
            # IVSDH composite score with new weights. The signs are important here:
            # - Positive Vega/Vomma: Increases sensitivity to IV changes (more risk).
            # - Vanna/Charm: Can be positive or negative, indicating directional bias changes due to IV/time. We use abs() to measure magnitude of the effect.
            ivsdh_score = (
                vega_intensity * 0.3 +          # Base volatility sensitivity
                abs(vanna_intensity) * 0.3 +  # Delta's sensitivity to IV (magnitude of change)
                abs(vomma_intensity) * 0.2 +  # Vega's sensitivity to IV (magnitude of convexity)
                abs(charm_intensity) * 0.1 +  # Delta's sensitivity to time (magnitude of decay effect)
                iv_differential * 0.1         # Raw difference in IV from underlying
            )
            ...
            self.logger.warning(f"Error calculating IVSDH scores: {e}")
    ...
    def _calculate_ugch_scores_optimized(self, df_strike: pd.DataFrame, und_data, symbol: str) -> pd.DataFrame:
        """
        Corrected and enhanced UGCH calculation, adhering to the EOTS Guide v2.5.
        This version now correctly includes Charm in the confluence of Greek forces.
        """
        try:
            # Extract all Greek exposures using FAIL-FAST validation.
            ...
            charm_exposure = self._get_column_required(df_strike, 'total_charmxoi_at_strike', 'for UGCH calculation') # Added Charm
            ...
            charm_norm = self._normalize_exposure_optimized(charm_exposure, 'charm', symbol) # Added Charm
            
            # UGCH composite score with adjusted weights to include Charm.
            # We use the absolute values because we are interested in the magnitude of the forces at a strike, not their direction.
            ugch_score = (
                abs(gamma_norm) * 0.25 + 
                abs(delta_norm) * 0.20 + 
                abs(vega_norm) * 0.20 + 
                abs(theta_norm) * 0.15 + 
                abs(vanna_norm) * 0.15 + 
                abs(charm_norm) * 0.05  # Added Charm with a smaller weight
            )
    ...
    def calculate_all_underlying_aggregates(
        self,
        df_strike: pd.DataFrame,
        und_data,
    ) -> UnderlyingAggregatesOutput:
        """
        ...
        try:
            ...
            # Return fail-fast Pydantic model instead of raw dict
            return UnderlyingAggregatesOutput.model_validate(aggregates)
